import React from 'react'
import {useDispatch} from 'react-redux'
import { done, edit } from '../features/todo';
import {useState} from 'react'
import "../App.css"
function Task({todo}) {
    const dispatch = useDispatch()
    const [editt, setEditt] = useState(false)
    const [disc, setDisc] = useState("")
    const editDisc = () =>{
        dispatch(edit({id: todo.id,disc:disc}))
        setEditt(false)
        
    }
  return (
    <div  className="task">
        {
            editt ? (
                <>
                <input type="text" defaultValue={todo.disc} onChange={(e) => setDisc(e.target.value)} />
                <button onClick={()=>editDisc() } >save</button>
                </>
            ):
            (
                <>
                <h4 className={todo.state ? "done":""} onClick={()=> setEditt(true)} >{todo.disc}</h4>
                <button onClick={()=> setEditt(true)} >edit</button>
                </>
            )
        }
   
    <button onClick={()=>dispatch(done(todo.id))} >
    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-check-square" viewBox="0 0 16 16">
      <path d="M14 1a1 1 0 0 1 1 1v12a1 1 0 0 1-1 1H2a1 1 0 0 1-1-1V2a1 1 0 0 1 1-1h12zM2 0a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V2a2 2 0 0 0-2-2H2z"/>
      <path d="M10.97 4.97a.75.75 0 0 1 1.071 1.05l-3.992 4.99a.75.75 0 0 1-1.08.02L4.324 8.384a.75.75 0 1 1 1.06-1.06l2.094 2.093 3.473-4.425a.235.235 0 0 1 .02-.022z"/>
    </svg>
    </button>
    </div>
  )
}

export default Task